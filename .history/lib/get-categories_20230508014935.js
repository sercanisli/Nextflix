...
export async function getStaticProps(idFromPaths) {
    ...
    const api = await axios(`${baseUrl}/popular?api_key=${process.env.API_KEY}&language=en-US&with_genres=${id}`)
...
export async function getStaticPaths () {
    const api = await axios(`https://api.themoviedb.org/3/genre/movie/list`);
