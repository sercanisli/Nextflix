    ...
    <div>
      ...
      <ul  > 
        {categories.map((category) => (
          <li key={category.id}>
            <button className="" onClick={() => handleCategoryChange(category)}>
              {category.name}
              </button>
          </li>
        ))}
      </ul>
      <h2>{selectedCategory && selectedCategory.name}<span className="text-white text-2xl mt-8 mb-5 "  >Movies</span> </h2>
